...
template <typename CharT>
class narrow_cfile_writer_factory
{
public:
    ...
    constexpr narrow_cfile_writer_factory(FILE* file) noexcept
        : _file(file)
    {}
...
class wide_cfile_writer_factory
{
public:
    ...
    constexpr wide_cfile_writer_factory(FILE* file) noexcept
        : _file(file)
    {}
...
inline auto write(std::FILE* destination)
{
    return stringify::v0::dispatcher_no_reserve
        < stringify::v0::detail::narrow_cfile_writer_factory<CharT> >
        (destination);
}
...
{
    return stringify::v0::dispatcher_no_reserve
        < stringify::v0::detail::wide_cfile_writer_factory >
        (destination);
}
